...
4. В окне клиента убедитесь, что подставился юзернейм бота, и нажмите **Подключиться**. Последние нажатия будут отображаться в окне; галочка **Копировать ввод** (по умолчанию выключена) включает/выключает воспроизведение нажатий локально. Лампочка рядом со статусом зеленеет, если в ответ на пинги `/t` за последние 10 секунд пришёл `/t` от бота. Кнопка **Тест** вручную отправляет `/t` и обновляет индикатор.
