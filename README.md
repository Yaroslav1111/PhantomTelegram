...
3. Откройте `logger/server.py` и заполните константы вверху файла:
   ```python
   BOT_TOKEN = "ТОКЕН_БОТА_СЕРВЕРА"
   TARGET_USER_ID = "ВАШ_USER_ID"
   ```
4. Запустите сервер: `python logger/server.py`
5. В Telegram напишите вашему боту `/start`. Бот будет присылать каждое нажатие. Командой `/stop` можно остановить скрипт.

## Настройка и запуск клиента (машина-приёмник)
1. Получите **API ID** и **API hash** на https://my.telegram.org.
2. Откройте `logger/client.py` и заполните константы вверху файла:
   ```python
   API_ID = 123456      # ваш api_id (int)
   API_HASH = "..."     # ваш api_hash (str)
   SESSION_NAME = "client_session"  # можно оставить как есть
   SERVER_BOT_USERNAME = "myserverbot"  # юзернейм сервер-бота без @
   ```
